...
function render() {
  ...
  chart.setOption({
    ...
    yAxis: [
      ...
      isSpeed
        ? { type: 'value', name: `${t('chart.axisSpeed')}(${props.speedUnit || 'KB/s'})`, splitLine: { show: false } }
...
// 数据、节点序列或语言变化都要重画:canvas 上的系列名/轴名是画上去的,
// 不重画就还是旧语言的文字。
watch(() => [props.data, props.agents, props.metric, props.speedUnit, locale.value], render, { deep: true })
</script>
